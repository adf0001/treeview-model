...
var verifyChildrenContainer = function (el, insertMode) {
	if (insertMode) {
		console.error("inserting fail on children container");
		return false;
	}

	if (!getNodeInfo(el)?.[INFO_TYPE]) {
...
/*
options:{ (outerHtml | innerHtml | name | nameHtml, toExpand, toExpandTemplate),
...
var addNode = function (elNode, options, childrenContainer) {
	...
	if (childrenContainer && !verifyChildrenContainer(elNode, options.insert)) return;
