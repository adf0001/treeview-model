...
//return el or elNode; return Error if fail;
var verifyChildrenContainer = function (el, insertMode, childrenContainer) {
	if (childrenContainer) {
		if (insertMode) {
			return Error("inserting fail on children container");
		}

		if (childrenContainer !== "true" && !getNodeInfo(el)?.[INFO_TYPE]) {
			return Error("children container type error");
		}
...
return the added node, or Error if fail;
*/
