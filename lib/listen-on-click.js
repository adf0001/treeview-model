...
var listenOnClick = function (el, options) {
	...
	/*
		getOptions:
			if evt is null and getOptions is true, return the options;
	*/
	container.onclick = function (evt, getOptions) {
		if (!evt && getOptions) return options;
		...
		if (elTarget.classList.contains("tree-to-expand")) {
			...
			if (!state || !updateSelection ||
				!(elChildren = nodeChildren(elTarget)) || !elChildren.children.length
			) return;
...
//get the `options` argument that last call .listenOnClick()
var getOnClickOptions = function (el) {
	return getContainer(el)?.onclick?.(null, true);
}
