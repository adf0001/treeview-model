...
return true if finished, or Error if fail;
*/
var removeNode = function (elNode, options) {
	if (elNode instanceof Array) {
		//for elNode array
		var anyReturn;
		elNode.forEach(v => { anyReturn = (removeNode(v, options) === true) || anyReturn; });
		return anyReturn;
	}
	...
	if (ni[INFO_TYPE] && !onlyChildren) return Error("fail on container");

	elNode = ni[INFO_NODE];

	//get dom parent element
	var elParent = onlyChildren ? (ni[INFO_TYPE] ? elNode : nodeChildren(elNode)) : elNode.parentNode;
	if (!elParent) return Error("fail to get parent node");
	...
	if (unselectList && updateSelection === "shift") {
		shiftList = unselectList.map(v => {
			v = v.nextElementSibling || v.previousElementSibling || v.parentNode.parentNode;
	...
	if (elParent && !elParent.firstElementChild &&
		ni[INFO_TYPE] !== "container" && elParent !== getContainer(elNode)) {
