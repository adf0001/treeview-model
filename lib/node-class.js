...
var { containerAttribute } = require("./container-attribute.js");
...
var getContainerClassNode = function (el, className) {
	...
	var v = containerAttribute(container, className + "-eid-data", void 0, true);
	if (!v) return;

	if (v instanceof Array) {
		var a = [], i, imax = v.length, imax0 = imax;

		for (i = 0; i < imax; i++) {
			el = document.getElementById(v[i]);

			//if element not exist, remove it;
			if (!el) {
				v.splice(i, 1);
				i--; imax--;
				continue;
		...
		if (imax !== imax0) element_attribute.setJson(container, className + "-eid-data", v);
		...
		//if element not exist, remove it;
		if (!el) element_attribute.remove(container, className + "-eid-data");
...
var isContainerClassNodeMultiple = function (el, className) {
...
module.exports = {
	...
	getClassNode: getContainerClassNode,

	isContainerClassNodeMultiple: isContainerClassNodeMultiple,
	isClassNodeMultiple: isContainerClassNodeMultiple,
